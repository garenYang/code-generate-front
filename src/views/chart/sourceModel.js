...
function initSourceFormBean (projectId) {
  const formBean = {
    ...
    entity: null,
    customColumnList: [],
    // 结构树 当前id: id 父节点id:parentId 父节点id列表：parentIds sort: 排序
    treeNodes: { id: null, parentId: null, parentIds: null, label: null, sort: null, leaf: null },
    id: null,
    parentId: null,
    parentIds: null,
    label: null,
...
function extractFormBean (form) {
  ...
  // 结构树合并treeNodes
  copy.treeNodes.id = copy.id
  copy.treeNodes.parentId = copy.parentId
  copy.treeNodes.parentIds = copy.parentIds
  copy.treeNodes.label = copy.label
  copy.treeNodes.sort = copy.sort
  copy.treeNodes.leaf = copy.leaf
  delete copy.id
...
function getRules () {
  return {
    ...
    id: [
      { required: true, message: '请设置当前节点ID', trigger: 'change' }
    ],
    parentId: [
      { required: true, message: '请设置父节点ID', trigger: 'change' }
    ],
    parentIds: [
      { required: true, message: '请设置父节点ID列表', trigger: 'change' }
    ],
    label: [
      { required: true, message: '请设置节点名称字段', trigger: 'change' }
    ],
    sort: [
      { required: true, message: '请设置节点排序字段', trigger: 'change' }
    ],
    leaf: [
      { required: true, message: '请设置是否叶子节点字段', trigger: 'change' }
    ]
  }
}
