      ...
      <el-row type="flex" align="middle" :gutter="20">
        <el-col :span="16">
          <el-form ref="sourceForm" class="sourceForm"
            ...
            <!-- 结构树当前节点(id) -->
            <el-form-item v-if="itemShow(['tree'], form.items)" label="当前节点ID字段" prop="id">
              <help-popover name="chartSource.treeNodes.id">
                <el-select v-model="form.id" value-key="key"
                           style="width:100%;" placeholder="当前节点ID字段"
                           >
                  <el-option-group
            ...
            <!-- 结构树父节点(parentId) -->
            <el-form-item v-if="itemShow(['tree'], form.items)" label="父节点ID" prop="parentId">
              <help-popover name="chartSource.treeNodes.parentId">
                <el-select v-model="form.parentId" value-key="key"
                           style="width:100%;" placeholder="请选择父节点ID字段"
                           >
                  <el-option-group
                    v-for="([joinIndex,entity]) in entityFieldOptions"
                    :key="joinIndex"
            ...
            <!-- 结构树父节点列表(parentIds) -->
            <el-form-item v-if="itemShow(['tree'], form.items)" label="父节点IDS" prop="parentIds">
              <help-popover name="chartSource.treeNodes.parentIds">
                <el-select v-model="form.parentIds" value-key="key"
                           style="width:100%;" placeholder="请选择父节点IDS字段">
                  <el-option-group
                    v-for="([joinIndex,entity]) in entityFieldOptions"
                    :key="joinIndex"
                    :label="entity.title+'('+entity.tableName+')'">
            ...
            <!-- 结构树节点名称字段(label) -->
            <el-form-item v-if="itemShow(['tree'], form.items)" label="节点名称" prop="label">
              <help-popover name="chartSource.treeNodes.label">
                <el-select v-model="form.label" value-key="key"
                           style="width:100%;" placeholder="请选择节点名称字段">
                  <el-option-group
                    v-for="([joinIndex,entity]) in entityFieldOptions"
                    :key="joinIndex"
                    :label="entity.title+'('+entity.tableName+')'">
            ...
            <!-- 结构树节点排序字段(sort) -->
            <el-form-item v-if="itemShow(['tree'], form.items)" label="节点排序" prop="sort">
              <help-popover name="chartSource.treeNodes.sort">
                <el-select v-model="form.sort" value-key="key"
                           style="width:100%;" placeholder="请选择节点排序字段">
                  <el-option-group
                    v-for="([joinIndex,entity]) in entityFieldOptions"
                    :key="joinIndex"
                    :label="entity.title+'('+entity.tableName+')'">
            ...
            <!-- 结构树节点排序字段(sort) -->
            <el-form-item v-if="itemShow(['tree'], form.items)" label="是否叶子节点" prop="leaf">
              <help-popover name="chartSource.treeNodes.leaf">
                <el-select v-model="form.leaf" value-key="key"
                           style="width:100%;" placeholder="请选择是否叶子节点字段">
                  <el-option-group
                    v-for="([joinIndex,entity]) in entityFieldOptions"
                    :key="joinIndex"
                    :label="entity.title+'('+entity.tableName+')'">
            ...
            <!-- 明细列：多选下拉框 -->
            <el-form-item v-if="itemShow(['detail'], form.items)" label="明细列" prop="detailColumnList">
            ...
            <!-- 自定义明细列 -->
            <el-form-item v-if="itemShow(['detail'], form.items)" label="自定义列">
            ...
            <!-- 过滤条件 -->
            <el-form-item v-if="itemShow(['filter'], form.items)" label="过滤">
            ...
            <!-- 排序 -->
            <el-form-item v-if="itemShow(['detail'], form.items)" label="排序">
            ...
            <!-- 聚合(维度) -->
            <el-form-item v-if="itemShow(['aggregation'], form.items)" label="聚合(维度)" prop="dimensionList">
            ...
            <!-- 聚合(指标) -->
            <el-form-item v-if="itemShow(['aggregation'], form.items)" label="聚合(指标)" prop="metricsList">
            ...
            <!-- 聚合(过滤) -->
            <el-form-item v-if="itemShow(['aggregation'], form.items) && form.metricsList.length>0" label="聚合(过滤)">
            ...
            <!-- 聚合(排序) -->
            <el-form-item v-if="itemShow(['aggregation'], form.items)" label="聚合(排序)">
...
export default {
  ...
  methods: {
    ...
    itemShow (showList, curShowList) {
      // curShowList 对应页面需要展示 item列表
      // showList 能在哪些页面展示 item列表
      for (var item of showList) {
        if (curShowList.indexOf(item) >= 0) {
          return true
        }
      }
      return false
    ...
    submit () {
      ...
      return this.$refs.sourceForm.validate()
        ...
        .then(() => {
          ...
          var result = model.extractFormBean(this.form)
          return chartSourceApi.saveOrUpdateWithItems(result, this.edit)
        })
        // 执行页面跳转
        .then(data => {
  ...
  created () {
    ...
    this.form.items = this.chartType.items
    ...
    if (this.sourceId) {
      ...
      promise.then(() => chartSourceApi.getWithItems(this.sourceId))
        .then(formBean => {
          console.log('formBean', formBean)
          formBean.items = this.chartType.items
