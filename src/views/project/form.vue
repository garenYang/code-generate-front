<template>
  <div class="projectFormDiv">
    <el-breadcrumb separator-class="el-icon-arrow-right">
      <el-breadcrumb-item :to="{ path: '/project' }">工程管理</el-breadcrumb-item>
      <el-breadcrumb-item>
        {{edit?'编辑工程':'创建工程'}}
      </el-breadcrumb-item>
    </el-breadcrumb>
      ...
      <el-col :span="12">
        <el-form ref="projectForm" class="projectForm"
          ...
          <el-form-item label="所属项目">
            <help-popover name="project.projectGroupId">
              <span>{{ edit ? this.old.projectGroupName:
                (projectGroupData  ? projectGroupData.projectGroupName: '')}}</span>
            </help-popover>
          </el-form-item>
          ...
          <el-form-item label="spring-boot版本" prop="feature.bootVersion">
            ...
            <el-form-item label="其他功能">
              ...
              <help-popover name="project.feature.other">
                <el-checkbox v-model="form.feature.nacosEnabled" tabindex="62">启用Nacos注册中心</el-checkbox>
              </help-popover>
              <help-popover name="project.feature.other">
                <el-checkbox v-model="form.feature.nacosConfigEnabled" tabindex="62">启用Nacos配置中心</el-checkbox>
              </help-popover>
            </el-form-item>
          </el-form-item>
...
export default {
  ...
  methods: {
    setProjectGroupId () {
      this.projectGroupData = this.$store.state.projectGroup
      if (!this.projectGroupData) {
        this.$common.showMsg('error', '请先选择项目')
        return false
      }
      return true
    },
    getProject () {
      ...
      return projectApi.get(this.projectId)
        .then(data => { this.old = data; console.log(this.old) })
    ...
    submit () {
      ...
      this.$refs.projectForm.validate()
        ...
        .then(() => {
          ...
          if (!this.edit) {
            this.form.projectGroupId = this.projectGroup.id
          } else {
            this.form.projectGroupId = this.old.projectGroupId
          }
