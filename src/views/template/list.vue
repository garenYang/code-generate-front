<template>
  <div class="templateList">
    ...
    <el-table v-loading="loading" :data="list"
      ...
      <el-table-column label="模板类型" width="78">
        <template v-slot="scope">
          <span v-if="scope.row.sysDefault" class="table-column-tag sys-default-tag">
            预置模板
          </span>
          <span v-if="!scope.row.sysDefault" class="table-column-tag sys-custome-tag">
            个性模板
          </span>
        </template>
      </el-table-column>
      <el-table-column label="模板编码" width="200">
        <template v-slot="scope">
          {{scope.row.code}}
        </template>
      </el-table-column>
      <el-table-column property="name" label="模板名称"></el-table-column>
      <el-table-column property="templateVersion" width="80" label="版本号"></el-table-column>
      <el-table-column property="sysLowVersion" width="80" label="最低兼容"></el-table-column>
      ...
      <el-table-column label="操作" width="200">
        <template v-slot="scope">
          ...
          <el-button @click="copyTemplate(scope.row)" type="text" size="medium">复制</el-button>
          <el-button v-if="!scope.row.sysDefault"
                     @click="handleEdit(scope.row)" type="text" size="medium">编辑</el-button>
          <el-button v-if="!scope.row.sysDefault"
                     @click="handleTemplateFile(scope.row)" type="text" size="medium">文件管理</el-button>
        </template>
      </el-table-column>
    </el-table>
